...
const SideNav = () => {
  ...
  return (
    <nav className="fixed z-50 bottom-4 left-1/2 -translate-x-1/2 md:bottom-auto md:left-auto md:translate-x-0 md:right-[max(8px,calc((100vw-1100px)/2+8px))] md:top-1/2 md:-translate-y-1/2 flex flex-row md:flex-col gap-3 md:gap-4">
        ...
        return (
          <div key={i}>
            <Link
              ...
              className={`w-[62px] h-[62px] md:w-[70px] md:h-[70px] rounded-[var(--radius)] flex items-center justify-center hover:scale-105 transition-transform ${active ? activeAccentClass : "bento-card"}`}
            >
      ...
      <button
        ...
        className="w-[62px] h-[62px] md:w-[70px] md:h-[70px] rounded-[var(--radius)] bento-card flex items-center justify-center hover:scale-105 transition-transform"
      >
        {isDark ? <Sun className="w-6 h-6 text-card-foreground" /> : <Moon className="w-6 h-6 text-card-foreground" />}
      </button>

      <div className="relative group h-[62px] md:h-[70px] w-[62px] md:w-[70px]">
        <div className="absolute left-0 top-0 h-full bento-card rounded-[var(--radius)] overflow-hidden flex items-center transition-[width] duration-300 w-[62px] md:w-[70px] group-hover:w-[220px] group-focus-within:w-[220px] md:group-hover:w-[228px] md:group-focus-within:w-[228px]">
          <button
            type="button"
            aria-label={labels.language}
            className="shrink-0 w-[62px] h-[62px] md:w-[70px] md:h-[70px] flex items-center justify-center border-r border-border"
          >
            <Languages className="w-6 h-6 text-card-foreground" />
          </button>

          <div
            className="shrink-0 pl-1 pr-2 flex items-center gap-1 opacity-0 translate-x-2 pointer-events-none transition-all duration-200 group-hover:opacity-100 group-hover:translate-x-0 group-hover:pointer-events-auto group-focus-within:opacity-100 group-focus-within:translate-x-0 group-focus-within:pointer-events-auto"
            role="menu"
            aria-label={labels.language}
          >
